...
const char restOfLine[] =  { 1, 0 };
 
static bool keepAliveCommand( struct CS_WebSocket *ws, struct UserState *userState, const char *line, int lineLength ) {
    char *savePtr;
    char *copy = tempCopyWithNulls( line, lineLength );
    strtok_r( copy, " ", &savePtr );
    char *howLong = strtok_r(NULL, " ", &savePtr );
    char *command = strtok_r(NULL, restOfLine, &savePtr );
    
    if( !howLong ) {
        NullStringToWebsockets( userState, ws, "Please specify time in seconds for the keepalive", true );
        return false;
    }
    if( !command ) {
        NullStringToWebsockets( userState, ws, "Specify a command for the keepalive.", true );
        return false;
    }

    int timeForKeepalive = atol(howLong);

    if( timeForKeepalive == 0 ) {
        NullStringToWebsockets( userState, ws, "Longer than 0 seconds please.", true );
        return false;
    }

    if( !userState->front ) {
        NullStringToWebsockets( userState, ws, "Need to have a mud", true );
        return false;
    }

    const char *commandWithCRLF = CS_tempBuffSnprintf( 256, "%s\r\n", command );
    
    char *commandToStuffIn = CS_cstringCopy(commandWithCRLF);
...
static struct commandToHandler commands[] = {
    ...
    { "/keepalive", 10, false, keepAliveCommand }
};
...
bool DoAllKeepalives() {
    CS_hashtableGrabMutex(cheapSessions);

    CS_HASHTABLE_ITER(cheapSessions, item) {
        struct UserState *user = (struct UserState *)item->value;
        DoKeepalives(user, true);
    }
    
    CS_hashtableReleaseMutex(cheapSessions);
    return false;
}
